...
public class Tracer {
            ...
            while (processRunning && (eventSet = vm.eventQueue().remove()) != null) {
                for (Event event : eventSet) {
                    if (event instanceof VMStartEvent) {
                        setupStepRequest(vm, targetClassName);
                    } else if (event instanceof StepEvent) {
                        StepEvent stepEvent = (StepEvent) event;
                        
                        // Heuristic: If we are about to execute a line that likely blocks for input,
                        // emit an input_waiting snapshot BEFORE resuming.
    ...
    private static void setupStepRequest(VirtualMachine vm, String targetClassName) {
